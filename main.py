...
class MyBot(commands.Bot):
    ...
    async def refresh_menu(self, guild_id, channel):
        # SIMULTANEOUS SMOOTH LOGIC
        if self.lock.locked(): return # Ignore if already moving
        ...
        async with self.lock:
            ...
            # 2. SEND NEW FIRST (Instant Appearance)
            new_msg = await channel.send(embed=embed, view=SubjectView())
            
            # 3. DELETE OLD SIMULTANEOUSLY (Background Task)
            if old_id:
                # We don't 'await' the delete, so it happens while the user looks at the new msg
    ...
    async def delete_message_safe(self, channel, msg_id):
        """Helper to delete messages without crashing the bot if they're already gone."""
        try:
